<template>
  ...
  <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mt-6">
    <div
        v-for="user in users"
        :key="user.id"
        class="bg-white dark:bg-gray-800 shadow-md dark:shadow-lg rounded-lg p-4"
    >
      <div class="flex items-center space-x-4">
        <img
            class="w-16 h-16 rounded-full border dark:border-gray-500"
            :src="user.image ? `http://192.168.11.71:8000/storage/${user.images[0]}` : 'https://cdn-icons-png.flaticon.com/512/3177/3177440.png'"
            alt="User Avatar"
        />
        <div>
          <h2 class="text-lg font-bold text-gray-800 dark:text-white">{{ user.full_name }}</h2>
          <p class="text-gray-600 dark:text-gray-300">{{ user?.roles?.map(role => role.name).join(', ') }}</p>
          <p class="text-gray-600 dark:text-gray-300">{{ user?.branches?.map(branch => branch.name).join(', ') }}</p>
      ...
      <div class="mt-4 flex justify-around">
        <router-link
            ...
            class="mr-1 transition text-white bg-blue-500 hover:bg-blue-600 dark:bg-blue-700 dark:hover:bg-gray-600 p-3 py-2 rounded duration-200"
        >
          Ko'rish
        </router-link>
        <button
            @click="openUpdateModal(user.id)"
            class="bg-green-500 text-white px-4 py-2 rounded hover:bg-green-600 dark:bg-green-700 dark:hover:bg-gray-600"
        >
          <i class="bx bxs-edit-alt"></i>
        </button>
        <button
            @click="deleteUser(user.id)"
            class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-600 dark:bg-red-700 dark:hover:bg-gray-600"
        >
