<template>
  ...
  <div class="flex flex-col items-center p-6 bg-gray-100 min-h-screen">
    <div class="w-full max-w-4xl bg-white shadow-md rounded-lg p-6">
      <div class="flex items-center space-x-6">
        <img
            ...
            alt="User Image"
            class="w-32 h-32 object-cover rounded-full border-4 border-gray-300"
        />
        <div>
          <h2 class="text-2xl font-semibold text-gray-900">{{ data?.full_name }}</h2>
          <p class="text-gray-500">{{ data?.email }}</p>
          <p class="text-gray-500" v-if="data?.phone">{{ data?.phone }}</p>
        </div>
      </div>

      <div class="grid grid-cols-2 gap-4 mt-6">
        <div class="p-3 bg-gray-50 rounded-lg shadow-sm" v-if="data?.companies && data?.companies.length>0">
          <span class="text-sm text-gray-500">Kompaniya</span>
          <p class="text-lg font-semibold text-gray-900" v-for="company in data?.companies">
            {{ company.name }}
          </p>
        </div>
        <div class="p-3 bg-gray-50 rounded-lg shadow-sm" v-if="data?.branches && data?.branches.length>0">
          <span class="text-sm text-gray-500">Filial</span>
          <p class="text-lg font-semibold text-gray-900" v-for="branch in data?.branches">
            {{ branch.name }}
          </p>
        </div>
        <div class="p-3 bg-gray-50 rounded-lg shadow-sm" v-if="data?.birthday">
          <span class="text-sm text-gray-500">Tug'ilgan sana</span>
          <p class="text-lg font-semibold text-gray-900">{{ data?.birthday }}</p>
        </div>
        <div class="p-3 bg-gray-50 rounded-lg shadow-sm" v-if="data?.work_start">
          <span class="text-sm text-gray-500">Ish boshlagan sana</span>
          <p class="text-lg font-semibold text-gray-900">{{ data?.work_start }}</p>
        </div>
        <div class="p-3 bg-gray-50 rounded-lg shadow-sm" v-if="data?.roles && data?.roles.length>0">
          <span class="text-sm text-gray-500">Lavozimi</span>
        ...
        <div class="p-3 bg-gray-50 rounded-lg shadow-sm">
          ...
          <p :class="data?.status === 'active' ? 'text-green-600' : 'text-red-600'" class="text-lg font-semibold">
      ...
      <div class="flex justify-end mt-6 space-x-2">
        <button @click="openUpdateSidebar" class="bg-green-500 hover:bg-green-600 text-white px-4 py-2 rounded">
    ...
    <div class="w-full max-w-4xl mt-6" v-if="data?.my_courses && data?.my_courses.length>0">
      <h3 class="text-xl font-semibold text-gray-900 mb-3">Kurslar</h3>
      <table class="w-full bg-white shadow-md rounded-lg overflow-hidden">
        ...
        <tbody>
        <tr v-for="(course, index) in data?.my_courses" :key="course.id" class="border-b hover:bg-gray-100">
          <td class="px-4 py-2">{{ index + 1 }}</td>
          <td class="px-4 py-2">{{ course.name }}</td>
          <td class="px-4 py-2">{{ course.price }}</td>
          <td class="px-4 py-2">{{ course.status }}</td>
          <td class="px-4 py-2 text-right">
            <router-link :to="{ name: 'WatchCourse', params: { id: course.id } }" class="text-blue-500">
