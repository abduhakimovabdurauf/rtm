<template>
  <nav class="flex z-[99] flex-col sm:flex-row items-center justify-between p-2 bg-white shadow-md w-full">
      ...
      <router-link to="/profile">
        <div class="flex items-center gap-2 p-2 bg-gray-100 hover:bg-blue-100 rounded-md cursor-pointer transition-all duration-300">
          ...
          <div class="flex flex-col">
            <span class="text-gray-800 font-medium text-base">{{ user.full_name }}</span>
            <span class="text-gray-800 text-sm">{{ user?.roles[0].name }}</span>
          </div>
        </div>
      </router-link>
